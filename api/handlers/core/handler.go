...
func (h *handlerCore) CreateUser(c *fiber.Ctx) error {
	...
	if err != nil || request.Name == "" || request.IdUser == "" || request.LastName == "" || request.Email == "" || request.Age == "" || request.Gender == "" || request.Password == "" {
		logger.Error.Printf("No se pudo parsear el cuerpo de la petición al modelo Request: %v", err)
		res.Code, res.Type, res.Msg = 1, "error", "La peticion realizada no es valida"
		return c.Status(http.StatusAccepted).JSON(res)
	}
   ...
   <soapenv:Body>
	...
	payload := fmt.Sprintf(template, request.IdUser, request.Name, request.LastName, request.Email, request.Age, request.Gender, request.Password)
